...
void Environment::setMemoryMaps(const struct retro_memory_map* maps) {
    ...
    LOGI("Captured %u memory descriptors from the core", maps->num_descriptors);
}

size_t Environment::readMemoryRegion(uint64_t address, size_t length, unsigned char* output) {
    std::lock_guard<std::mutex> lock(memoryDescriptorsLock);
    for (auto& desc : memoryDescriptors) {
...
size_t Environment::writeMemoryRegion(uint64_t address, size_t length, const unsigned char* input) {
    std::lock_guard<std::mutex> lock(memoryDescriptorsLock);
    for (auto& desc : memoryDescriptors) {
        if (desc.ptr == nullptr) continue;
...
bool Environment::callback_set_sensor_state(unsigned port, enum retro_sensor_action action, unsigned rate) {
    ...
    switch (action) {
        case RETRO_SENSOR_ACCELEROMETER_ENABLE: env.sensorMask |= 1u; return true;
        case RETRO_SENSOR_ACCELEROMETER_DISABLE: env.sensorMask &= ~1u; return true;
        case RETRO_SENSOR_GYROSCOPE_ENABLE: env.sensorMask |= 2u; return true;
        case RETRO_SENSOR_GYROSCOPE_DISABLE: env.sensorMask &= ~2u; return true;
        case RETRO_SENSOR_ILLUMINANCE_ENABLE: env.sensorMask |= 4u; return true;
        case RETRO_SENSOR_ILLUMINANCE_DISABLE: env.sensorMask &= ~4u; return true;
        default: return false;
    }
}

float Environment::callback_sensor_get_input(unsigned port, unsigned id) {
    auto& env = Environment::getInstance();
    return id < 7 ? env.sensorValues[id] : 0.0f;
}

void Environment::setSensorValues(float ax, float ay, float az, float gx, float gy, float gz, float lux) {
    sensorValues[0] = ax; sensorValues[1] = ay; sensorValues[2] = az;
    sensorValues[3] = gx; sensorValues[4] = gy; sensorValues[5] = gz;
    sensorValues[6] = lux;
}
...
bool Environment::handle_callback_environment(unsigned cmd, void *data) {
    switch (cmd) {
        ...
        case RETRO_ENVIRONMENT_GET_SENSOR_INTERFACE: {
            ...
            auto* iface = (struct retro_sensor_interface*) data;
            iface->set_sensor_state = &callback_set_sensor_state;
            iface->get_sensor_input = &callback_sensor_get_input;
            return true;
        }
        ...
        // IronMON One patch: the tracker's window into emulated memory.
        case RETRO_ENVIRONMENT_SET_MEMORY_MAPS:
            LOGD("Called RETRO_ENVIRONMENT_SET_MEMORY_MAPS");
            setMemoryMaps(static_cast<const struct retro_memory_map*>(data));
            return true;
