...
// IronMON One patch: read emulated memory captured via SET_MEMORY_MAPS. Returns an
// empty array when the address range is not covered - callers treat that as "core has
// no map here", never as zeroed memory.
JNIEXPORT jbyteArray JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_readMemory(
    JNIEnv* env,
    jclass obj,
    jlong address,
    jint length
    ...
    jbyteArray result = env->NewByteArray(static_cast<jsize>(read));
    if (read > 0) {
        env->SetByteArrayRegion(result, 0, static_cast<jsize>(read),
                                reinterpret_cast<jbyte*>(buffer.data()));
    }
    return result;
}

// IronMON One patch: write emulated memory (B-to-Run cursor injection). Returns bytes
// written; 0 = unmapped or read-only region.
JNIEXPORT jint JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_writeMemory(
    JNIEnv* env,
    jclass obj,
    jlong address,
    jbyteArray data
...
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_step(
    ...
    // KaizoCore patch: drain the achievements client's queues into Kotlin.
    {
        auto& ch = Cheevos::getInstance();
        if (ch.isCreated()) {
            CheevosServerCall call;
            while (ch.popServerCall(call)) {
                jclass cls = env->GetObjectClass(glRetroView);
                jmethodID m = env->GetMethodID(cls, "cheevosServerCall", "(ILjava/lang/String;Ljava/lang/String;Ljava/lang/String;)V");
                jstring url = env->NewStringUTF(call.url.c_str());
                jstring post = env->NewStringUTF(call.postData.c_str());
                jstring ct = env->NewStringUTF(call.contentType.c_str());
                env->CallVoidMethod(glRetroView, m, call.id, url, post, ct);
                env->DeleteLocalRef(url); env->DeleteLocalRef(post); env->DeleteLocalRef(ct);
            }
            CheevosEvent ev;
            while (ch.popEvent(ev)) {
                jclass cls = env->GetObjectClass(glRetroView);
                jmethodID m = env->GetMethodID(cls, "cheevosEvent", "(ILjava/lang/String;Ljava/lang/String;ILjava/lang/String;I)V");
                jstring t = env->NewStringUTF(ev.title.c_str());
                jstring d = env->NewStringUTF(ev.description.c_str());
                jstring b = env->NewStringUTF(ev.badgeUrl.c_str());
                env->CallVoidMethod(glRetroView, m, ev.type, t, d, ev.points, b, ev.result);
                env->DeleteLocalRef(t); env->DeleteLocalRef(d); env->DeleteLocalRef(b);
            }
        }
    }
}
...
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_cheevosLogin(JNIEnv* env, jclass obj, jstring user, jstring password, jboolean isToken) {
    if (isToken) Cheevos::getInstance().loginWithToken(jstr(env, user), jstr(env, password));
    else Cheevos::getInstance().loginWithPassword(jstr(env, user), jstr(env, password));
}
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_cheevosLogout(JNIEnv* env, jclass obj) { Cheevos::getInstance().logout(); }
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_cheevosSetHardcore(JNIEnv* env, jclass obj, jboolean on) { Cheevos::getInstance().setHardcore(on); }
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_cheevosLoadGame(JNIEnv* env, jclass obj, jstring path, jint consoleId) {
    Cheevos::getInstance().loadGame(jstr(env, path), (unsigned) consoleId);
}
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_cheevosUnloadGame(JNIEnv* env, jclass obj) { Cheevos::getInstance().unloadGame(); }
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_cheevosReset(JNIEnv* env, jclass obj) { Cheevos::getInstance().reset(); }
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_cheevosIdle(JNIEnv* env, jclass obj) { Cheevos::getInstance().idle(); }
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_cheevosServerResponse(JNIEnv* env, jclass obj, jint id, jstring body, jint status) {
    Cheevos::getInstance().serverResponse(id, jstr(env, body), status);
}
JNIEXPORT jstring JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_cheevosSummary(JNIEnv* env, jclass obj) {
    return env->NewStringUTF(Cheevos::getInstance().summaryJson().c_str());
}
JNIEXPORT jstring JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_cheevosAchievements(JNIEnv* env, jclass obj) {
    return env->NewStringUTF(Cheevos::getInstance().achievementsJson().c_str());
}
...
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_setSensorValues(
    JNIEnv* env, jclass obj, jfloat ax, jfloat ay, jfloat az, jfloat gx, jfloat gy, jfloat gz, jfloat lux
) {
    Environment::getInstance().setSensorValues(ax, ay, az, gx, gy, gz, lux);
}

JNIEXPORT jint JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_getSensorMask(JNIEnv* env, jclass obj) {
    return (jint) Environment::getInstance().getSensorMask();
}

JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_setSlowMotion(
    JNIEnv* env,
    jclass obj,
    jint divisor
) {
