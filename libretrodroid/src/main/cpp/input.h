...
private:
    struct GamePadState {
        // IronMON One: key events arrive from the app's UI thread while the emu
        // thread polls every frame. The upstream unordered_set is not safe for
        // that (a concurrent rehash can strand the reader in an infinite bucket
        // walk, which presents as the whole core freezing). Retro joypad ids are
        // all < 64, so an atomic bitmask makes both sides lock-free.
        std::atomic<uint64_t> pressedMask { 0 };
