...
void Input::onKeyEvent(unsigned int port, int action, int keyCode) {
    ...
    if (retroKeyCode < 0 || retroKeyCode >= 64) {
        return;
    }
    uint64_t bit = uint64_t(1) << retroKeyCode;
    if (action == AKEY_EVENT_ACTION_DOWN) {
        pads[port].pressedMask.fetch_or(bit, std::memory_order_relaxed);
    } else if (action == AKEY_EVENT_ACTION_UP) {
        pads[port].pressedMask.fetch_and(~bit, std::memory_order_relaxed);
    }
}
...
bool Input::anyPressed(unsigned int port, unsigned int id) const {
    ...
    uint64_t mask = pads[port].pressedMask.load(std::memory_order_relaxed);
